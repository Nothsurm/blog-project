...
export default function PostCard({post}) {
  ...
  return (
    <div className="group relative w-full border border-teal-500 hover:border-2 transition-all h-[350px] overflow-hidden rounded-lg sm:w-[350px]">
        <Link to={`/post/${post.slug}`}>
            <img 
                src={post.image} 
                alt="post cover" 
                className="h-[200px] w-full object-cover hover group-hover:h-[140px] transition-all duration-300 z-20" 
            />
        </Link>
