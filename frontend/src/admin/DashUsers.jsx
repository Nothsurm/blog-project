...
export default function DashUsers() {
  ...
  const handleShowMore = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) {
        setUses((prevUsers) => [...prevUsers, ...data.users])
        if (data.users.length < 9) {
          setShowMore(false)
        }
      }
            ...
            {users.map((user) => (
              <Table.Body className='divide-y' key={user._id}>
                <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                  ...
                  <Table.Cell>
                      <img src={user.profilePicture} alt={user.username} className='w-14 h-14 object-cover bg-gray-500 rounded-full'/>
                  </Table.Cell>
                  <Table.Cell>
                      {user.username}
                  </Table.Cell>
          ...
          </Table>
          {
            showMore && (
              <div className="text-center">
                <button onClick={handleShowMore} className='text-teal-500 text-sm py-7'>
                  Show more
                </button>
              </div>
            )
